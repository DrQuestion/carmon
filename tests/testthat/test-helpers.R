test_that("layers check function returns layers unchanged if rownames are identical and nrow is equal", {
  m1 <- matrix(1:4, nrow = 2, dimnames = list(c("A", "B"), NULL))
  m2 <- matrix(5:8, nrow = 2, dimnames = list(c("A", "B"), NULL))
  layers <- list(m1, m2)
  result <- check_layers_dims(layers)
  expect_equal(result, layers)
})

test_that("layers check function reorders layers if rownames match but are in different order", {
  m1 <- matrix(1:4, nrow = 2, dimnames = list(c("A", "B"), NULL))
  m2 <- matrix(5:8, nrow = 2, dimnames = list(c("B", "A"), NULL))
  layers <- list(m1, m2)
  result <- check_layers_dims(layers)
...
test_that("layers check function stops if rownames differ across layers", {
  m1 <- matrix(1:4, nrow = 2, dimnames = list(c("A", "B"), NULL))
  m2 <- matrix(5:8, nrow = 2, dimnames = list(c("C", "D"), NULL))
  layers <- list(m1, m2)
  expect_error(check_layers_dims(layers), "Samples should be along rows")
})

test_that("layers check function uses colnames if ncol matches but not nrow, returns just transposed if order same", {
  m1 <- matrix(1:4, nrow = 2, dimnames = list(NULL, c("A", "B")))
  m2 <- matrix(5:10, nrow = 3, dimnames = list(NULL, c("A", "B")))
  layers <- list(m1, m2)
  expect_warning(result <- check_layers_dims(layers), "samples will be assumed to be distributed along the columns")
  expect_equal(result, lapply(layers, t))
})

test_that("layers check function uses colnames, reorders if same colnames in different order and transposes them", {
  m1 <- matrix(1:4, nrow = 2, dimnames = list(NULL, c("A", "B")))
  m2 <- matrix(5:10, nrow = 3, dimnames = list(NULL, c("B", "A")))
  layers <- list(m1, m2)
  expect_warning(result <- check_layers_dims(layers), "samples will be assumed to be distributed along the columns")
  expect_equal(rownames(result[[2]]), c("A", "B"))
})
...
test_that("layers check function stops if neither nrow nor ncol matches", {
  ...
  layers <- list(m1, m2)
  expect_error(check_layers_dims(layers), "Dimensions do not match among layers")
})
...
test_that("layers number check function stops if number is inconsistent", {
  ...
  omics <- c("a", "b")
  marginals <- c("a", "b", "c")
  expect_error(capture.output(check_layers_num(layers, omics = omics, marginals = marginals)),
               "Number of layers is inconsistent across input parameters. Please check your input.")
})

test_that("omics check function raises warnings for non implemented omics and returns FALSE", {
  expect_warning(check <- check_omics(omics = c("foo-seq", "baromics"), marginals = c(0,0)))
  expect_false(check)
})
...
test_that("unnamed features are renamed after their omics type", {
  ...
  expected_names <- c("rnaseq_1", "rnaseq_2", colnames(multi_omics_micro[[2]]))
  names(expected_names) <- names(actual_names)
  expect_equal(actual_names, expected_names)
})

test_that("which_omics work", {
  expect_snapshot(which_omics())
})

test_that("which_marginals work", {
