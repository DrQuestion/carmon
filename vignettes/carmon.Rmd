...
In this vignette we explain how to use the R package Copula-Aided Reconstruction of Multi-Omics Networks (carmon). The aim of the package is to reconstruct networks from two or more layers of multi-omics data, harnessing the flexibility of copulas to transfer non-normally distributed omics data to the normal realm.
...
dataset_1 <- multi_omics_small$rnaseq
dataset_2 <- multi_omics_small$metabolomics

# This shows that observations are on the rows for both data sets:
dim(dataset_1) # [1] 30 14
dim(dataset_2) # [1] 30  5

# This shows that the observations are all in the same order in both data sets:
all(rownames(dataset_1) == rownames(dataset_2)) # TRUE

# We can now build our R list as follows:
multi_omics_small <- list("rnaseq" = dataset_1, "metabolomics" = dataset_2)
```
